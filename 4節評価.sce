...
eta20=15000; eta30=13000; eta40=10000;//勾配法の学習率(初期値)

E_sp=0.0000001; E_sp_q=E_sp^2;//勾配法の際、探索を終了する傾き、その2乗
...
function j=fj(x,u)   //現在の状態変数から一定のトルクで動かす予測し、評価を出力する（評価関数）
    q1=x(1);
    q2=x(2);
    q3=x(3);
    q4=x(4);
...
function ans=gradent(left,right,x)//勾配法で評価関数から最適解を求める
    /*
    //解の上限と下限を決めるプログラム
    f_left=fj(x,left);
    f_right=fj(x,right);
...
for i=lookT*dt:lookT*dt:endT//リンクを描写した時間に縦線を引く
    xplt=[i,i];
    zplt=[0.3,-0.3];
    plot(xplt,zplt,':b');
end
